...
uint16_t LSM9DS1::begin()
{
	...
	// Now, initialize our I2C hardware interface.
	if (_xgAddress != 0)
	{
		xgI2c.init(1, _xgAddress);

		// To verify communication, we can read from the WHO_AM_I register
		uint8_t xgTest = xgReadByte(WHO_AM_I_XG);
		if (xgTest != WHO_AM_I_AG_RSP) return 0;
	}

	if (_mAddress != 0)
	{
		mI2c.init(1, _mAddress);

		// To verify communication, we can read from the WHO_AM_I register
		uint8_t mTest = mReadByte(WHO_AM_I_M);
		if (mTest != WHO_AM_I_M_RSP) return 0;
	}
...
{
	if (_xgAddress == 0) return;
	xgI2c.writeByte(subAddress, data);
}

void LSM9DS1::mWriteByte(uint8_t subAddress, uint8_t data)
{
	if (_mAddress == 0) return;
	mI2c.writeByte(subAddress, data);
}

uint8_t LSM9DS1::xgReadByte(uint8_t subAddress)
{
	if (_xgAddress == 0) return -1;
	return xgI2c.readByte(subAddress);
}

uint8_t LSM9DS1::xgReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	if (_xgAddress == 0) return -1;
	return xgI2c.readBytes(subAddress, dest, count);
}

uint8_t LSM9DS1::mReadByte(uint8_t subAddress)
{
	if (_mAddress == 0) return -1;
	return mI2c.readByte(subAddress);
}

uint8_t LSM9DS1::mReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	if (_mAddress == 0) return -1;
	return mI2c.readBytes(subAddress, dest, count);
}
